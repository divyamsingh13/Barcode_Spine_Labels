...
final class Barcode39 {
	...
	public function draw($filename = null) {
		...
		// $_new= imagecolorallocate($img, 45, 45, 45);
		// check if using barcode text
		if($this->barcode_text) {
			$vab = str_replace("*", "", $barcode_string);
			...
			// draw barcod text
			//imagestring($img, $font_size, $pos_center, $this->barcode_height - $barcode_text_h - 5,
				//$vab, $_000);
			
			
			  // Create image instances
			  $dest = imagecreatefrompng('main.png');
			  //$src = imagecreatefrompng($img);
			  
			  // Copy and merge
			  //imagecopymerge($dest, $img, 15, 35, 0, 0, 137, 80, 100);
			  imagecopymerge($dest, $img, 15, 35, 0, 0, 137, 50, 100);
